...
export default {
  ...
  data() {
    return {
      api: {
        basepath: `http://${process.env.VUE_APP_API_HOSTNAME}:${process.env.VUE_APP_API_PORT}/api/v1`
      },
      meta: {
  ...
  methods: {
    ...
    getData() {
      const vm = this
      axios.get(vm.api.basepath + '/zones', {
        auth: {
        ...
        vm.zones = res.data
        axios.get(vm.api.basepath + '/schedules', {
          auth: {
            username: process.env.VUE_APP_API_USERNAME,
            password: process.env.VUE_APP_API_PASSWORD
          }
    ...
    createSchedule() {
      ...
      if (errors.length > 0) {
        ...
        axios.post(vm.api.basepath + '/schedules',
          schedule, {
            auth: {
              username: process.env.VUE_APP_API_USERNAME,
              password: process.env.VUE_APP_API_PASSWORD
